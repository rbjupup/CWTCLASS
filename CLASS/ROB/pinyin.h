...
///////////////////////////////////////////拼音结束/////////////////////////////////////////
CString change2PinYin(CString str) {
	char * outbuf = new char[HZ2PY_OUTPUT_BUF_ARRAY_SIZE];
	memset(outbuf, '\0', sizeof(char) * HZ2PY_OUTPUT_BUF_ARRAY_SIZE);
	if (is_utf8_string(str.GetBuffer())) {
		printf("utf8:");
		pinyin_utf8(str.GetBuffer(), outbuf);
	} else {
		printf("gb2312:");
		pinyin_gb2312(str.GetBuffer(), outbuf, false, false, true, true, true);
	}
	CString resultstr(outbuf);
	delete outbuf;
	return resultstr;
}