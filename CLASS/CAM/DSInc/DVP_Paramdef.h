...
typedef struct
{
    ...
    INT   iFieldCount;          // 传感器扫描方式,
                                // 1为逐行扫描，大于1的为对应场数的隔行扫描
} tDSSensorInfo;
...
typedef struct
{
    UINT    uiMediaType;        // 图像数据类型，
                                // 参考“DVP_Define.h”头文件中枚举类型“emDSDataType”的值
    UINT    uBytes;             // 图像数据字节数
    UINT    uiHeight;           // 图像高度
    UINT    uiWidth;            // 图像宽度
    UINT    uHOff;              // 图像水平偏移量
...
typedef struct
{
	INT		iError;     // 错误码，相机通信错误时，通过回调返回此错误码
	LPVOID  pUser;      // 用户自定义数据指针
}tDSCallback;
