						...
						int iTab, BOOL bIsActive, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if (bIsActive || bIsHighlight)
	{
		...
		{
			clrTab = pTabWnd->IsMDITab() ? m_clrHighlightDn : RGB(255, 255, 255);
		}
		...
		else
		{
			...
			{
				dm.FillGradient (rectFill, m_clrHighlightedTabGradient1, clr2);
			}
			else
			{
				dm.FillGradient (rectFill, clr2, m_clrHighlightedTabGradient1);
			}
		}
		...
		{
			CRect rectBottom = rectTab;
			rectBottom.right--;
			rectBottom.top = rectBottom.bottom;
			rectBottom.bottom += 3;

			pDC->FillSolidRect(rectBottom, clrTab);
		}
		else
		{
	...
	OnDrawTabContent (pDC, rectTab, iTab, bIsActive, pTabWnd, bIsActive ? RGB(0, 0, 0) : RGB(255, 255, 255));
}
//*********************************************************************************************************
...
int CBCGPVisualManagerVS2010::GetTabExtraHeight(const CBCGPTabWnd* pTabWnd)
{
	...
	return 2 * globalData.GetTextMargins ();
}
//*************************************************************************************
...
BOOL CBCGPVisualManagerVS2010::OnDrawPushButton (CDC* pDC, CRect rect, CBCGPButton* pButton, COLORREF& clrText)
{
	...
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || pButton->m_bOnGlass ||
