...
public:
	...
	void SetTreatNulls(CBCGPChartSeries::TreatNulls tn, BOOL bRecalcMinMax = TRUE);
	...
	BOOL SetLegendID(int nLegendID, int nDataPointIndex = -1, BOOL bCreateIfLegendNotFound = FALSE, 
	...
	BOOL IsComponentSet(CBCGPChartData::ComponentIndex ci) const 
	{
		return ci == CBCGPChartData::CI_X ? m_bXComponentSet : m_bZComponentSet;
	}

	BOOL IsXComponentSet() const {return m_bXComponentSet;}
	BOOL IsZComponentSet() const {return m_bZComponentSet;}
...
public:
	...
	void SetStockSeriesType(CBCGPBaseChartStockSeries::StockSeriesType type);
	...
	CBCGPBaseChartStockSeries* GetChildSeries(int nIdx);
