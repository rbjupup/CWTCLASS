...
void CImagePaintArea::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	pDC->FillRect (&rectClient, &brGray);
	...
	{
		pDC->MoveTo (x, rectClient.top + 1);
		pDC->LineTo (x, rectClient.bottom - 1);
	}

	for (y = rectDraw.top + m_sizeCell.cy; 
		y <= rectDraw.bottom - m_sizeCell.cy; y += m_sizeCell.cy)
	{
		pDC->MoveTo (rectClient.left + 1, y);
		pDC->LineTo (rectClient.right - 1, y);
	}
