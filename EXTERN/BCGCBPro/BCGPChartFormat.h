...
struct BCGCBPRODLLEXPORT BCGPChartFormatLabel : public BCGPChartFormatArea
{
	...
	void Reset()
	{
		...
		m_fOrgFontSize = m_textFormat.GetFontSize();
	...
	virtual void OnScaleRatioChanged(const CBCGPSize& sizeScaleRatioNew, const CBCGPSize& sizeScaleRatioOld)
	{
		if (m_szScaleRatio.cx == 1. && m_szScaleRatio.cy == 1)
		{
			m_fOrgFontSize = m_textFormat.GetFontSize();
		}
		...
		m_textFormat.SetFontSize(m_fOrgFontSize * (float)sizeScaleRatioNew.cy);
	}
...
struct BCGCBPRODLLEXPORT BCGPChartFormatLegendTable : public BCGPChartFormatArea
{
	...
	void Reset()
	{
		...
		m_bAutoSetDirection = TRUE;
		m_bIsVariableEntrySize = FALSE;
	}
	...
	// in horizontal style entries are vertical
	BOOL				m_bIsHorizontal;
	// horz and vert direction set automatically by related chart control
	// shared legends must be set manually
	BOOL				m_bAutoSetDirection;
