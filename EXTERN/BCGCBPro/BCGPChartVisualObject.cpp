													...
													BOOL bRedraw, BOOL bByZoom)
{
	if (bByZoom)
	{
		for (int i = 0; i < dataStart.GetComponentCount(); i++)
		{
			...
			if (dblStart > dblEnd)
			{
				dataStart.SetValue(dblEnd, (CBCGPChartData::ComponentIndex) i);
				dataEnd.SetValue(dblStart, (CBCGPChartData::ComponentIndex) i);
			}
...
void CBCGPChartVisualObject::AdjustLayout(CBCGPGraphicsManager* pGM)
{
	...
	BOOL bCustomLegendRect = OnGetLegendAreaRect(rectLegend);
...
BOOL CBCGPChartVisualObject::OnMouseDown(int nButton, const CBCGPPoint& pt)
{
	...
	if (IsScrollEnabled() && nButton == 0)
	{
		...
		if ((hitInfo.m_hitInfo & BCGPChartHitInfo::HIT_AXIS_SCROLL_BAR) != 0)
		{
			...
			{
				ASSERT_VALID(pAxis);
				pAxis->ScrollTo(pAxis->ValueFromPointByScrollRange(pt));
				SetDirty(TRUE, TRUE);
				return TRUE;
			}
		}
		...
		{
			if (BeginThumbTrack(hitInfo.m_nIndex1))
			{
				return TRUE;
			}
		}
...
void CBCGPChartVisualObject::OnMouseMove(const CBCGPPoint& pt)
{
	...
	else if (m_bPanMode && m_bEnableScroll)
	{
		...
		{
			CBCGPSize szScroll;
			szScroll.SetSize(m_ptPanStart.x - pt.x, pt.y - m_ptPanStart.y);
			...
			for (int i = 0; i < m_arAxes.GetSize(); i++)
			{
				...
				double dblStart = pAxis->ValueFromPoint(m_ptPanStart);
				...
				if (pAxis->Scroll(dblValScroll, TRUE))
				{
					bScrolled = TRUE;
...
BOOL CBCGPChartVisualObject::OnSetMouseCursor(const CBCGPPoint& pt)
{
	...
	for (int i = 0; i < m_arAxes.GetSize(); i++)
	{
		...
		if ((rectBounds.PtInRect(pt) || bDiagram3D) && (m_bEnableMagnifier || m_bEnableZoom || m_bEnablePan))
		{
			...
			BOOL bCanSetMagnifyCursor = (hitInfo.m_hitInfo & BCGPChartHitInfo::HIT_AXIS_SCROLL_BAR) == 0;
			...
			if ((m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_POINT) != 0)
			{
				if (!IsChart3D())
				{
					for (int j = 0; j < pSeries->GetDataPointCount(); j++)
					{
						...
						CRect rectBounds = pSeries->GetDataPointBoundingRect(j);
...
double CBCGPChartVisualObject::OnGetWallThickness() const
{
	...
	if (pAxis != NULL)
	{
		dblThickness = max(dblThickness, pAxis->GetMajorTickMarkLen(FALSE));
	}
									...
									BOOL bNoFill, BOOL bNoLine)
{
	...
	if (!bNoFill && colors.pBrFill != NULL && !colors.pBrFill->IsEmpty())
	{
		pGM->FillEllipse(ellipse, *colors.pBrFill);
	}
	
	if (!bNoLine && colors.pBrLine != NULL && !colors.pBrLine->IsEmpty())
	{
		pGM->DrawEllipse(ellipse, *colors.pBrLine, format.m_outlineFormat.GetLineWidth(TRUE), 
			&format.m_outlineFormat.m_strokeStyle);
	}
}
										...
										BOOL bNoFill, BOOL bNoLine)
{
	...
	if (!bNoFill && colors.pBrFill != NULL)
	{
		pGM->FillGeometry(geometry, *colors.pBrFill);
	}
	
	if (!bNoLine && colors.pBrLine != NULL)
	{
		pGM->DrawGeometry(geometry, *colors.pBrLine, format.m_outlineFormat.GetLineWidth(TRUE), 
			&format.m_outlineFormat.m_strokeStyle);
	}
}
//****************************************************************************************
							...
							const CString& strText, CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	if (pDP == NULL || colors.m_pBrDataLabelTextColor == NULL)
	{
		return;
	}
										...
										CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	if (pFormatSeries->m_curveType != BCGPChartFormatSeries::CCT_NO_LINE)
	{
		...
		dblLineWidth = bcg_clamp(dblLineWidth, 1., 7.);
											   ...
											   CBCGPChartSeries* pSeries, int nDataPointIndex, BOOL bAsMainItem)
{
	...
	int nMarkerSize = (int)ceil(markerFormat.GetMarkerSize().cy + markerFormat.m_outlineFormat.GetLineWidth(TRUE));
	...
	CBCGPPoint ptCenter((long)ptMarkerCenter.x + 0.5, (long)ptMarkerCenter.y + 0.5);
	double dMarkerSize2 = bcg_round(nMarkerSize / 2.0);

	if (pGM->GetType() == CBCGPGraphicsManager::BCGP_GRAPHICS_MANAGER_GDI)
	{
		dMarkerSize2 = nMarkerSize / 2;
	...
	case BCGPChartMarkerOptions::MS_RHOMBUS:
		{
			...
			arPoints[0] = CBCGPPoint(ptCenter.x, ptCenter.y - dMarkerSize2);
			arPoints[1] = CBCGPPoint(ptCenter.x + dMarkerSize2, ptCenter.y);
			arPoints[2] = CBCGPPoint(ptCenter.x, ptCenter.y + dMarkerSize2);
			arPoints[3] = CBCGPPoint(ptCenter.x - dMarkerSize2, ptCenter.y);
	...
	case BCGPChartMarkerOptions::MS_TRIANGLE:
		{
			...
			CBCGPPoint ptTop(ptCenter.x, ptCenter.y - dMarkerSize2);  
...
//****************************************************************************************
BOOL CBCGPChartVisualObject::AddChartEmptyData(int nSeries, DWORD dwUserData, BCGPChartFormatSeries* pFormatDataPoint, 
									   BCGPChartFormatSeries* pFormatSeries)
{
	ASSERT_VALID (this);
	...
	CBCGPChartDataPoint dataPoint;
	dataPoint.m_dwUserData = dwUserData;

	if (pFormatDataPoint != NULL)
	{
		dataPoint.SetFormat(*pFormatDataPoint);
	}
	...
	return pSeries->AddDataPoint(dataPoint);
}
//****************************************************************************************
...
// Legend support
//****************************************************************************************
BOOL CBCGPChartVisualObject::AddPrivateLegend(CBCGPChartLegend* pNewLegend)
{
	ASSERT_VALID(this);
	ASSERT_VALID(pNewLegend);

	if (GetLegend(pNewLegend->GetLegendID()) != NULL)
	{
		ASSERT(FALSE);
		TRACE1("A legend with ID %d has already been added", pNewLegend->GetLegendID());
		return FALSE;
	}

	m_lstPrivateLegends.AddTail(pNewLegend);
	return TRUE;
}

CBCGPChartLegend* CBCGPChartVisualObject::CreatePrivateLegend(int nID, CRuntimeClass* pLegendRTC)
{
	ASSERT_VALID(this);

	if (GetLegend(nID) != NULL)
	{
		ASSERT(FALSE);
		TRACE1("A legend with ID %d has already been added", nID);
		return NULL;
	}
	
	CBCGPChartLegend* pNewLegend = NULL;

	if (pLegendRTC != NULL && !pLegendRTC->IsDerivedFrom(RUNTIME_CLASS(CBCGPChartLegend)))
	{
		ASSERT(FALSE);
		TRACE0("CBCGPChartVisualObject::CreatePrivateLegend: The specified runtime class is not derived from CBCGPChartLegend.");
		return NULL;
	}

	if (pLegendRTC != NULL)
	{
		pNewLegend = DYNAMIC_DOWNCAST(CBCGPChartLegend, pLegendRTC->CreateObject());
		ASSERT_VALID(pNewLegend);

		pNewLegend->SetLegendID(nID);
		pNewLegend->SetHostControl(GetOwner());
	}
	else
	{
		pNewLegend = new CBCGPChartLegend(nID, GetOwner());
	}
...
int CBCGPChartVisualObject::GetPrivateLegendCount()
{
	ASSERT_VALID(this);
	return (int)m_lstPrivateLegends.GetCount();
}

BOOL CBCGPChartVisualObject::RemovePrivateLegend(int nID, int nTransferLegendID)
{
	ASSERT_VALID(this);

	CBCGPChartLegend* pTransferLegend = GetLegend(nTransferLegendID);

	// verify that we transfer entries to a valid legend
	if (nTransferLegendID != -1 && pTransferLegend == NULL)
	{
		return FALSE;
	}

	CBCGPChartLegend* pRemovedLegend = GetPrivateLegend(nID);
	if (pRemovedLegend != NULL)
	{
		// set all IDs belong to new legend. ALL Custom entries will go away!
		pRemovedLegend->SetLegendID(nTransferLegendID);

		POSITION pos = m_lstPrivateLegends.Find(pRemovedLegend);
		ASSERT(pos != NULL);
		m_lstPrivateLegends.RemoveAt(pos);
		delete pRemovedLegend;
	}

	return TRUE;
}

CBCGPChartLegend* CBCGPChartVisualObject::GetPrivateLegend(int nID)
{
	ASSERT_VALID(this);

	for (POSITION pos = m_lstPrivateLegends.GetHeadPosition(); pos != NULL;)
	{
		CBCGPChartLegend* pLegend = DYNAMIC_DOWNCAST(CBCGPChartLegend, m_lstPrivateLegends.GetNext(pos));
		if (pLegend == NULL)
		{
			continue;
		}
...
CBCGPChartLegend* CBCGPChartVisualObject::GetSharedLegend(int nID)
{
	ASSERT_VALID(this);

	for (POSITION pos = m_lstSharedLegends.GetHeadPosition(); pos != NULL;)
	{
		CBCGPChartLegend* pLegend = DYNAMIC_DOWNCAST(CBCGPChartLegend, m_lstSharedLegends.GetNext(pos));
		if (pLegend == NULL)
		{
			continue;
		}
...
CBCGPChartLegend* CBCGPChartVisualObject::GetLegend(int nID)
{
	ASSERT_VALID(this);

	CBCGPChartLegend* pLegend = GetPrivateLegend(nID);

	if (pLegend == NULL)
	{
		return GetSharedLegend(nID);
	}
									  ...
									  const CBCGPRect& rectDiagram)
{
	...
	rectText.DeflateRect(pAxis->m_axisLabelsFormat.GetContentPadding(TRUE));
													...
													CBCGPChartSeries* pSeries) const
{
	...
	if (valX.IsEmpty() || pSeries->IsIndexMode())
	{
...
void CBCGPChartVisualObject::Zoom(int nMagnifier, const CBCGPPoint& ptZoomCenter)
{
	...
	else
	{
		for (int i = 0; i < m_arAxes.GetSize(); i++)
		{
			...
			if (rectBounds.PtInRect(ptZoomCenter))
			{
