...
int CBCGPDlgImpl::OnCreate()
{
	...
	if (pDialog != NULL || pPropSheet != NULL)
	{
		if ((dwStyle & WS_CHILD) != WS_CHILD)
		{
			...
			if (m_bVisualManagerNCArea && IsOwnerDrawCaption() && !globalData.DwmIsCompositionEnabled ())
			{
...
BOOL CBCGPDlgImpl::LoadPlacement(LPCTSTR lpszProfileName)
{
	...
	{
		m_Dlg.SetWindowPos (&CWnd::wndTop, rectNormal.left, rectNormal.top, rectNormal.Width (), rectNormal.Height (),
 			SWP_NOZORDER | SWP_NOOWNERZORDER);
	}

	m_bWindowPlacementIsSet = TRUE;
	return TRUE;
}
//****************************************************************************
