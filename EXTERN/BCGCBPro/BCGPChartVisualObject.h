...
struct BCGCBPRODLLEXPORT BCGPChartHitInfo
{
	enum HitInfoTest
	{
		...
		HIT_AXIS_THUMB = 0x1000,	// index1 - axis ID, index2 set to -1
		HIT_AXIS_RESIZE_BAND = 0x2000,	// index1 - axis ID, index2 - 1: resize top; 0: resize bottom
		HIT_OBJECT = 0x4000,		// index1 - object ID
		HIT_ALL_AXIS_ELEMENTS = HIT_AXIS | HIT_AXIS_SCROLL_BAR | HIT_AXIS_THUMB | HIT_AXIS_RESIZE_BAND | HIT_AXIS_NAME,
...
// Legend
	CObList m_lstPrivateLegends;
	CObList m_lstSharedLegends;
...
// Series Management
	...
	BOOL AddChartEmptyData (int nSeries = 0, DWORD dwUserData = 0, BCGPChartFormatSeries* pFormatDataPoint = NULL, BCGPChartFormatSeries* pFormatSeries = NULL);
...
// Legends
	// returns a legend with specified ID
	// the chart creates the first private legend by default with ID 0
	CBCGPChartLegend* GetPrivateLegend(int nID = 0);
	BOOL AddPrivateLegend(CBCGPChartLegend* pNewLegend);
	CBCGPChartLegend* CreatePrivateLegend(int nID, CRuntimeClass* pLegendRTC = NULL);
	int GetPrivateLegendCount();
	// if nTransferLegendID is specified all entries are transfered to a legend with that ID
	BOOL RemovePrivateLegend(int nID, int nTransferLegendID = -1);

	CBCGPChartLegend* GetSharedLegend(int nID = 0);
	CBCGPChartLegend* GetLegend(int nID);
...
public:
	...
	virtual BOOL OnGetLegendAreaRect(CBCGPRect& rectLegendArea) const
	{
		rectLegendArea = m_rectLegendArea;
		return FALSE;
	}
