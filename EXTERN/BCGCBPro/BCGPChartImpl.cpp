...
IMPLEMENT_DYNCREATE(CBCGPStockChartImpl, CBCGPBaseChartImpl)
											 ...
											 CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	if (pDataPoint != NULL && !pSeries->IsDataPointScreenPointsEmpty(nDataPointIndex))
	{
		...
		pDataPoint->m_rectBounds = CBCGPRect(pt.x, pt.y, pt.x , pt.y);
	}
}
//****************************************************************************************
											...
											CBCGPPoint& ptMarkerPos, CBCGPRect& rectBounds)
{
	...
	*ppFormatSeries = (BCGPChartFormatSeries*) pDataPoint->GetFormat();
	...
	if (!(*ppFormatSeries)->m_dataLabelFormat.m_options.m_bShowDataLabel)
	{
		return FALSE;
	}
											  ...
											  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	{
		double dblLabelAngle = bcg_deg2rad(pFormatSeries->m_dataLabelFormat.m_options.m_dblAngle);
		...
		double dblDistance = pFormatSeries->m_dataLabelFormat.m_options.GetDistanceFromMarker() +
...
void CBCGPBaseChartImpl::ArrangeSides3D(CList<CBCGPChartSide3D*, CBCGPChartSide3D*>& lstSides)
{
	...
	for (POSITION pos = lstSides.GetHeadPosition(); pos != NULL;)
	{
		...
		for (POSITION posCheck = pos; posCheck != NULL;)
		{
			...
			{
// 				if (fabs(pSide->m_vPlaneCoef[2]) < DBL_EPSILON || fabs(pSideCheck->m_vPlaneCoef[2]) < DBL_EPSILON)
// 				{
// 					zSide = pSide->m_dblAverageZOrder;
// 					zSideCheck = pSideCheck->m_dblAverageZOrder;
// 				}
...
void CBCGPLineChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	{
		int nFirstIndex = 0;
		int nLastIndex = pSeries->GetDataPointCount() - 1;
		...
		for (int i = nFirstIndex; i <= nLastIndex; i++)
		{
			...
			if (i + 1 <= pSeries->GetMaxDataPointIndex())
			{
				...
				{
					if (!pSeries->IsDataPointScreenPointsEmpty(i) && !pSeries->IsDataPointScreenPointsEmpty(i + 1))
					{
		...
		if (pLineSeries != NULL && pLineSeries->IsConnectFirstLastPoints())
		{
			...
			OnDrawDiagramLine(pGM, pSeries, nLastIndex, nFirstIndex, pDataPointEnd, pDataPointStart, curveType);
			...
			if (pLineSeries->IsFillClosedShape() && nLastIndex > 1)
			{
				...
				if (curveType == BCGPChartFormatSeries::CCT_STEP || curveType == BCGPChartFormatSeries::CCT_REVERSED_STEP)
				{
					...
					for (int i = nFirstIndex; i < nLastIndex; i++)
					{
						if (!pSeries->IsDataPointScreenPointsEmpty(i) && !pSeries->IsDataPointScreenPointsEmpty(i + 1))
						{
							CBCGPPoint ptBottomStart = pSeries->GetDataPointScreenPoint(i, 0);
							CBCGPPoint ptBottomEnd = pSeries->GetDataPointScreenPoint(i + 1, 0);

							OnGetStepPoint(ptBottomStart, ptBottomEnd, pXAxis->IsVertical(), curveType, ptStep);
							linePoints.Add(ptBottomStart);
							linePoints.Add(ptStep);
						}
					}

					CBCGPPoint ptBottomStart = pSeries->GetDataPointScreenPoint(nLastIndex, 0);
					CBCGPPoint ptBottomEnd = pSeries->GetDataPointScreenPoint(nFirstIndex, 0);
					OnGetStepPoint(ptBottomStart, ptBottomEnd, pXAxis->IsVertical(), curveType, ptStep);
					linePoints.Add(ptBottomStart);
					linePoints.Add(ptStep);
				}
				...
				CBCGPPolygonGeometry geometry(linePoints, TRUE);
				m_pRelatedChart->OnDrawChartSeriesItem(pGM, geometry, pSeries, -1, 
			 ...
			 curveType == BCGPChartFormatSeries::CCT_SPLINE_HERMITE)
	{
		...
		int i = 0;
		for (i = 0; i < pSeries->GetDataPointCount(); i++)
		{
			if (!pSeries->IsDataPointScreenPointsEmpty(i))
			{
				linePoints.Add(pSeries->GetDataPointScreenPoint(i, 0));
		...
		if (nCount > 0)
		{
			if (nCount >= 3)
			{
				if (pLineSeries->IsConnectFirstLastPoints() && linePoints[0] != linePoints[nCount - 1])
				{
					CBCGPPoint pt = linePoints.GetAt(0); 
					linePoints.Add(pt);
				}

				CBCGPSplineGeometry splineGeometry(linePoints, splineType, pLineSeries->IsConnectFirstLastPoints());
				m_pRelatedChart->OnDrawChartSeriesItem(pGM, splineGeometry, pSeries, -1, 
					CBCGPChartVisualObject::CE_MAIN_ELEMENT, !pLineSeries->IsFillClosedShape(), FALSE);
			}
			else
			{
				CBCGPPolygonGeometry geometry(linePoints, pLineSeries->IsConnectFirstLastPoints());
				m_pRelatedChart->OnDrawChartSeriesItem(pGM, geometry, pSeries, -1, 
					CBCGPChartVisualObject::CE_MAIN_ELEMENT, !pLineSeries->IsFillClosedShape(), FALSE);
			}
		}
		...
		CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	// CBCGPChartDataPoint::m_rectBounds is used for data labels in 3D charts
	if (nDataPointIndex == pSeries->GetDataPointCount() - 1)
	{
		CBCGPChartShape3D* pShapePrev = pSeries->GetDataPointShape3D(nDataPointIndex - 1);
		...
		if (pShapePrev != NULL)
		{
			...
			{
				pDataPoint->m_rectBounds.right = pDataPoint->m_rectBounds.left;
				pDataPoint->m_rectBounds.bottom = pDataPoint->m_rectBounds.top;
			}
			else
			{
				pDataPoint->m_rectBounds.left = pDataPoint->m_rectBounds.right;
	...
	CBCGPPoint pt = pSeries->GetDataPointScreenPoint(nDataPointIndex, 0);
	CBCGPPoint ptNext = pSeries->GetDataPointScreenPoint(nDataPointIndex + 1, 0);
...
void CBCGPAreaChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	int i = 0;
	int nMinDataPointIndex = pSeries->GetMinDataPointIndex();
	int nMaxDataPointIndex = pSeries->GetMaxDataPointIndex();

	CBCGPPoint ptFirstScreenPoint;
	CBCGPPoint ptLastScreenPoint;
	for (i = nMinDataPointIndex; i <= nMaxDataPointIndex; i++)
	{
		const CBCGPChartDataPoint* pDataPoint = pSeries->GetDataPointAt(i);
		CBCGPPoint ptScreenPoint = pSeries->GetDataPointScreenPoint(i, 0);

		if (i == nMinDataPointIndex && !bIsStacked)
		{
		...
		if (pDataPoint != NULL && !pDataPoint->IsEmpty() || pSeries->IsVirtualMode())
		{
			...
			{
				CBCGPPoint ptNextPoint = pSeries->GetDataPointScreenPoint(i + 1, 0);
				if (curveType == BCGPChartFormatSeries::CCT_STEP)
				{
		...
		if (i == nMaxDataPointIndex && !bIsStacked)
		{
			if (!pAxisHorz->IsVertical())
			{
	...
	if (bIsStacked)
	{
		for (i = nMaxDataPointIndex; i >= nMinDataPointIndex ; i--)
		{
			const CBCGPChartDataPoint* pDataPoint = pSeries->GetDataPointAt(i);
			if (pDataPoint != NULL && !pDataPoint->IsEmpty())
			{
										  ...
										  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	if (pSeries->GetDataPointCount() == 2)
	{
		...
		pDataPoint->m_rectBounds.right = pt.x;
		pDataPoint->m_rectBounds.bottom = pt.y;
	}
}
		...
		CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	CBCGPPoint pt = pSeries->GetDataPointScreenPoint(nDataPointIndex, 0);
	CBCGPPoint ptNext = nDataPointIndex < nDataPointCount - 1 ? pSeries->GetDataPointScreenPoint(nDataPointIndex + 1, 0) : pt;
	...
	if (nDataPointCount > 2)
	{
		...
		else if (nDataPointIndex == nDataPointCount - 2)
		{
			if (pXAxis->IsVertical())
			{
				if (bReverseOrder)
...
void CBCGPBarChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	for (int i = pSeries->GetMinDataPointIndex(); i <= pSeries->GetMaxDataPointIndex(); i++)
	{
		...
		if (pDataPoint != NULL)
		{
			m_pRelatedChart->OnDrawChartSeriesItem(pGM, pDataPoint->m_rectBounds, pSeries, i, CBCGPChartVisualObject::CE_MAIN_ELEMENT, FALSE, FALSE);
		}
	}
}
//****************************************************************************************
...
CBCGPPoint CBCGPBarChartImpl::OnGetMarkerPoint(const CBCGPChartDataPoint* pDataPoint, CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	CBCGPRect rectBounds = pDataPoint->m_rectBounds;
	CBCGPChartAxis* pXAxis = pSeries->GetRelatedAxis(CBCGPChartSeries::AI_X);
									...
									CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	{
		dblXUnitSize = pXAxis->GetFixedIntervalWidth();
	}
	else if (pXAxis->m_bFormatAsDate)
	{
		dblXUnitSize *= pXAxis->GetMajorUnit();
	}
	...
	if (!pXAxis->IsVertical())
	{
		pDataPoint->m_rectBounds.SetRect(point.x, 
	...
	{
		pDataPoint->m_rectBounds.SetRect(ptBottom.x < point.x ? point.x : ptBottom.x, 
			point.y, 
	...
	{
		if (pXAxis->IsVertical() && pDataPoint->m_rectBounds.left == pDataPoint->m_rectBounds.right && 
			pDataPoint->m_rectBounds.left != 0.)
		{
			pDataPoint->m_rectBounds.right += 1.;
		}
		else if (!pXAxis->IsVertical() && pDataPoint->m_rectBounds.top == pDataPoint->m_rectBounds.bottom && 
			pDataPoint->m_rectBounds.bottom != 0.)
		{
			pDataPoint->m_rectBounds.top -= 1.;
		}
	}
		...
		!pBarSeries->IsCustomOffset() || !bIsGrouped)
	{
		if (pXAxis->IsVertical())
		{
			pDataPoint->m_rectBounds.OffsetRect(0, -dblBarWidth / 2);
		}
		else
		{
			pDataPoint->m_rectBounds.OffsetRect(-dblBarWidth / 2, 0);
		}
	}
	else if (!pBarSeries->IsCustomOffset() && nVisibleSeriesCount > 1)
	{
		if (!pXAxis->IsVertical())
		{
			if (nSeriesIndex == 0)
			{
				pDataPoint->m_rectBounds.right += dblOverlapDiff;
			}
			else if (nSeriesIndex == nVisibleSeriesCount - 1)
			{
				pDataPoint->m_rectBounds.left -= dblOverlapDiff;
			}
			else
			{
				pDataPoint->m_rectBounds.InflateRect(dblOverlapDiff / 2, 0);
			}
		}
		else
		{
			if (nSeriesIndex == 0)
			{
				pDataPoint->m_rectBounds.bottom += dblOverlapDiff;
			}
			else if (nSeriesIndex == nVisibleSeriesCount - 1)
			{
				pDataPoint->m_rectBounds.top -= dblOverlapDiff;
			}
			else
			{
				pDataPoint->m_rectBounds.InflateRect(0, dblOverlapDiff / 2);
			}
		}
	}
	...
	if (pDataPoint->m_rectBounds.Width() != 0 && pDataPoint->m_rectBounds.Height() != 0 && !m_pRelatedChart->IsChart3D())
	{
		pDataPoint->m_rectBounds.IntersectRect(rectDiagramArea);

		double dblValue = pDataPoint->GetComponentValue(CBCGPChartData::CI_Y);
		
		if (pXAxis->IsVertical())
		{
			if (pDataPoint->m_rectBounds.right == dblZeroLinePos && dblValue < 0)
			{
				pDataPoint->m_rectBounds.right -= 1.;
			}
			else if (pDataPoint->m_rectBounds.left == dblZeroLinePos && dblValue > 0)
			{
				pDataPoint->m_rectBounds.left += 1.;
			}
		}
		else
		{
			if (pDataPoint->m_rectBounds.bottom == dblZeroLinePos && dblValue > 0)
			{
				pDataPoint->m_rectBounds.bottom -= 1.;
			}
			else if (pDataPoint->m_rectBounds.top == dblZeroLinePos && dblValue < 0)
			{
				pDataPoint->m_rectBounds.top += 1.;
			}
		}
...
void CBCGPBubbleChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	for (int i = nMinValue; i <= nMaxValue; i++)
	{
		...
		if (pDataPoint == NULL || pDataPoint->m_rectBounds.IsRectEmpty())
		{
			continue;
		}
		...
		m_pRelatedChart->OnDrawChartSeriesMarker(pGM, pDataPoint->m_rectBounds.CenterPoint(), 
							  ...
							  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	else
	{
		...
		double dblDistance = dblDistanceFromMarker + pFormatSeries->m_dataLabelFormat.m_options.GetDistanceFromMarker() * 
			dblDistanceFromMarker / 100.;
								...
								CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	pDataPoint->m_rectBounds.SetRect(ptCenter, ptCenter);
	pDataPoint->m_rectBounds.InflateRect(nSize, nSize);
}
							  ...
							  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	double dblDistance = max(dblRadiusX, dblRadiusY) / 2 *  pFormatSeries->m_dataLabelFormat.m_options.GetDistanceFromMarker() / 100;
					...
					const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	double dblDistance = pFormatSeries->m_dataLabelFormat.m_options.GetDistanceFromMarker();
	double dblAngle = pFormatSeries->m_dataLabelFormat.m_options.m_dblAngle;
...
void CBCGPStockChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	double dblCandleWidth = (dblXUnitSize * pMainStockSeries->m_nOpenCloseBarSizePercent / 100.) / 2.; 
										...
										int nDataPointIndex, BOOL bUp)
{
	...
	{
		if ((int)rectCandle.Width() % 2 == 0)
		{
			rectCandle.left -= 0.5;
			rectCandle.right += 0.5;
		}
