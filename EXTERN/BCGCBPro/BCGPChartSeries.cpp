										 ...
										 BOOL bSetDefaultValue, double dblDefaultYValue)
{
	...
	if (bEnable)
	{
		...
		m_pXAxis->SetFixedMajorUnit(1., TRUE);
		...
		m_pXAxis->SetIndexedSeries(this);
		RecalcMinMaxValues();
		m_pXAxis->SetScrollRange(0, m_nHistoryDepth);
	}
}
//****************************************************************************************
	...
	if (m_pYAxis != NULL)
	{
			 ...
			 m_pYAxis->GetMaxDisplayedValue() > GetMaxValue(CBCGPChartData::CI_Y))
		{
			m_pYAxis->SetScrollRange(min(m_pYAxis->GetMinScrollValue(), m_pYAxis->GetMinDisplayedValue()), 
									 max(m_pYAxis->GetMaxScrollValue(), m_pYAxis->GetMaxDisplayedValue()));
		}
	}
}
//****************************************************************************************
...
int CBCGPChartSeries::GetMaxDataPointIndex() const
{
	...
	if (m_pXAxis != NULL && m_pXAxis->IsFixedMajorUnit() && m_bIndexMode)
	{
		return (int)min(GetDataPointCount() - 1, m_pXAxis->GetMaxDisplayedValue());
	}
	
	return GetDataPointCount() - 1;
}
//****************************************************************************************
...
void CBCGPChartSeries::OnCalcScreenPointsStacked(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	for (int i = 0; i < GetDataPointCount(); i++)
	{
		...
		if (pDp == NULL || pDp->IsEmpty() && m_treatNulls != CBCGPChartSeries::TN_VALUE)
		{
			continue;
		}
...
void CBCGPChartSeries::CopyFrom(const CBCGPChartSeries& src)
{
	...
	m_pXAxis = src.GetRelatedAxis(CBCGPChartSeries::AI_X);
	m_pYAxis = src.GetRelatedAxis(CBCGPChartSeries::AI_Y);
	m_pZAxis = src.GetRelatedAxis(CBCGPChartSeries::AI_Z);
}
//****************************************************************************************
...
//****************************************************************************************
BOOL CBCGPChartSeries::SetLegendID(int nLegendID, int nDataPointIndex, BOOL bCreateIfLegendNotFound, CRuntimeClass* pLegendRTC)
{
	ASSERT_VALID(this);

	if (m_nLegendID != nLegendID)
	{
		CBCGPChartLegend* pLegend = m_pChart->GetLegend(m_nLegendID);

		if (pLegend != NULL)
		{
			pLegend->RemoveLegendEntry(this, -1);
		}
	}
	...
	if (pLegend == NULL)
	{
		if (bCreateIfLegendNotFound)
		{
			CBCGPChartLegend* pLegend = NULL;
			if (pLegendRTC != NULL)
			{
				CObject* p = pLegendRTC->CreateObject();
				ASSERT_VALID(p);
				...
				pLegend = DYNAMIC_DOWNCAST(CBCGPChartLegend, p);

				if (pLegend == NULL)
				{
					ASSERT(FALSE);
					TRACE0("Invalid runtime class of chart legend specified for CBCGPChartSeries::SetLegendID");
					delete p;
					return FALSE;
				}
			}
			...
			{
				pLegend = new CBCGPChartLegend();
			}
		}
	}

	if (pLegend != NULL && m_pChart != NULL)
	{
		ASSERT_VALID(m_pChart);

		pLegend->SetHostControl(m_pChart->GetOwner());
		m_pChart->AddPrivateLegend(pLegend);

		pLegend->AddLegendEntry(this, nDataPointIndex);
	}
...
{
	m_bUpdateAxesOnNewData = TRUE;
// 	m_nFixedGridSizeX = 10;
// 	m_nValuesPerIntervalX = 5;
...
void CBCGPChartPieSeries::OnCalcScreenPoints(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	for (i = 0; i < GetDataPointCount(); i++)
	{
		...
		{
			dblMaxDistance = max(dblMaxDistance, pDataPointFormat->m_dataLabelFormat.m_options.GetDistanceFromMarker());
			bOutside = TRUE;
		}
...
//****************************************************************************************
CBCGPBaseChartStockSeries* CBCGPChartStockSeries::GetChildSeries(int nIdx)
{
...
void CBCGPChartStockSeries::EnableHistoryMode(BOOL bEnable, int nHistoryDepth, BOOL bReverseOrder, BOOL /*bSetDefaultValue*/, double /*dblDefaultYValue*/)
{
	...
	{
		ASSERT_VALID(m_pXAxis);
		m_pXAxis->SetFixedMajorUnit(1., FALSE);
	}
...
void CBCGPChartPolarSeries::OnCalcScreenPoints(CBCGPGraphicsManager* /*pGM*/, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	for (int i = GetMinDataPointIndex(); i <= GetMaxDataPointIndex(); i++)
	{
		...
		if (pDp != NULL)
		{
			...
			if (valY.IsEmpty() || valAngle.IsEmpty() && pXAxis->IsComponentXSet())
			{
				continue;
			}
...
void CBCGPChartTernarySeries::OnCalcScreenPoints(CBCGPGraphicsManager* /*pGM*/, const CBCGPRect& /*rectDiagramArea*/)
{
	for (int i = 0; i < GetDataPointCount(); i++)
	{
		...
		if (pDp->IsEmpty() || 
			pDp->GetComponentValue(CBCGPChartData::CI_Y) == 0 && pDp->GetComponentValue(CBCGPChartData::CI_Y1) == 0 && 
			pDp->GetComponentValue(CBCGPChartData::CI_Y2) == 0)
		{
