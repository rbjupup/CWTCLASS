...
void CBCGPDrawManager::DrawArc(const CRect& rect, double dblStartAngle, double dblFinishAngle, BOOL bIsClockwise, COLORREF clrLine)
{
	if (clrLine == -1)
	{
		return;
	...
	if (size.cx <= 2 || size.cy <= 2)
	{
		if (size.cx > 0 && size.cy > 0)
		{
			if (size.cx <= 2)
			{
				...
				DrawLine(x, rect.top, x, rect.bottom, clrLine);
			}
			else
			{
				int y = (rect.top + rect.bottom) / 2;
				DrawLine(rect.left, y, rect.right, y, clrLine);
			}
		}
