...
void CBCGPChartSide3D::OnDraw(CBCGPEngine3D* pGM, BOOL bFill, BOOL bDrawLine)
{
	...
	const CBCGPBrush& brFill = bFill ? *m_pBrFill : CBCGPBrush();
	const CBCGPBrush& brLine = bDrawLine ? *m_pBrLine : CBCGPBrush();
	...
	if (arVertexes.GetSize() == 4)
	{
		...
		else
		{
			...
			pGM->DrawSide(arVertexes[0], arVertexes[1], arVertexes[2], arVertexes[3], 
					brFill, CBCGPBrush(), m_pOutlineFormat->m_dblWidth, bFill, FALSE);
...
BOOL CBCGPChartSide3D::HitTest(const CBCGPPoint& ptHit, CBCGPChartShape3D** ppShape)
{
	...
	*ppShape = NULL;
	...
	if (/*bcg_pointInPolygon(arVertexes.GetData(), (int)arVertexes.GetSize(), ptHit)*/rect.PtInRect(ptHit))
	{
		*ppShape = m_pShape;
		return TRUE;
	}
	...
	if (pBrLine != NULL && pOutlineFormat != NULL)
	{
		for (POSITION pos = m_lst3DRectEdges.GetHeadPosition(); pos != NULL;)
		{
			...
			pGM->DrawLine(m_arFinalVertexes[edge.m_nIndex1], m_arFinalVertexes[edge.m_nIndex2], *pBrLine, 
				pOutlineFormat->m_dblWidth/*, &pOutlineFormat->m_strokeStyle*/);
		}
	}
}
//*******************************************************************************
...
// 	*ppBrFill = (CBCGPBrush*)(pDiagram3D->m_formatWalls.m_brBottomFillColor.IsEmpty() ? 
